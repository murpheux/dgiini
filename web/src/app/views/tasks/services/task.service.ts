...
export class TaskService {
    ...
    getFeaturedTasks(): Observable<IResponse> {
        ...
        const params = new HttpParams().set('filter', JSON.stringify({status: 'open'}));
    ...
    acceptBid(id: Guid): Observable<IResponse> {
        const url = `${this.bidServiceUrl}`;
        return this.http.put<IResponse>(url, { id, accepted: true });
    }

    acceptBid2(id: string, task: string): Observable<IResponse> {
        const url = `${this.bidServiceUrl}`;
        return this.http.put<IResponse>(url, { id, task, accepted: true });
    }
